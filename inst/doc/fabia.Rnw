...
\begin{enumerate}
\item Usage: \verb+ spfabia(X,p=5,alpha=0.1,cyc=500,spl=0,spz=0.5,non_negative=0,+\\
\hspace*{6em}\verb+ random=1.0,write_file=1,norm=1,scale=0.0,lap=1.0,nL=0)+
...
\end{enumerate}
  ...
  \item first line: numer of columns (the samples).

  \item second line: number of rows (the features).

  \item following lines: for each sample (column) three lines with

    \begin{enumerate}

    \item number of nonzero row elements

    \item indices of the nonzero row elements

    \item values of the nonzero row elements
