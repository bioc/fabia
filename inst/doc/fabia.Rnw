...
%setwd("c:/sepp/work/fabia/fabia_10/fabia/inst/doc")
%Sweave("fabia.Rnw")
...
\item {\bf \large FABIA:} perform  \Rfunction{fabia} (sparseness by Laplace prior)
  to extract biclusters; 13 biclusters, sparseness 0.1 (Laplace),
  400 cycles:
\begin{Sinput}
R> resToy1 <- fabia(X,13,0.1,400,1.0,1.0)
\end{Sinput}
\item Plot some results:
\begin{Sinput}
R> rToy1 <- extractPlot(resToy1,ti="FABIA",Y=Y)
\end{Sinput}
...
\item {\bf \large FABIAS:} perform  \Rfunction{fabias} (sparseness by projection)
  ...
  400 cycles:
\begin{Sinput}
R> resToy2 <- fabias(X,13,0.6,400,1.0)
\end{Sinput}
\item  Plot some results:
\begin{Sinput}
R> rToy2 <- extractPlot(resToy2,ti="FABIAS",Y=Y)
\end{Sinput}
\item Extract the biclusters:
\begin{Sinput}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=0.9\textwidth]{vectors_1}}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{fabia-lap1}
\includegraphics[width=0.45\textwidth]{fabia-lap2}
\end{center}
...
The functions \Rfunction{fabia} and  \Rfunction{fabias} are
...
The interface between \R and C is realized by the package \Rpackage{Rcpp}
\citep{Samperi:06}.
...
\item Usage:
\texttt{fabi(X,p=5,alpha=0.1,cyc=500,spl=1.0,spz=1.0,center=2,norm=1)}
...
\begin{enumerate}
\item Usage:
\texttt{fabia(X,p=5,alpha=0.1,cyc=500,spl=1.0,spz=1.0,random=1.0,center=2,norm=1,scale=0.0,lap=1.0,nL=0)}
...
resEx <- fabia(X,3,0.1,50,1.0,1.0)
...
\item Usage: \verb+fabiap(X,p=5,alpha=0.1,cyc=500,spl=1.0,spz=1.0,sL=0.6,sZ=0.6,+\\
\hspace*{6em}\verb+   random=1.0,center=2,norm=1,scale=0.0,lap=1.0,nL=0)+
...
\end{align}
  ...
  The code is implemented in C using the Rcpp package. The projection is
  implemented in \R.
...
resEx <- fabiap(X,3,0.1,50,1.0,1.0,0.6,0.6)
...
\item  Usage:
\texttt{fabias(X,p=5,alpha=0.6,cyc=500,spz=1.0,random=1.0,center=2,norm=1,lap=1.0,nL=0)}
...
\item  Usage:
\texttt{fabiasp(X,p=5,alpha=0.6,cyc=500,spz=1.0,center=2,norm=1)}
...
resToy <- fabiasp(X,13,0.6,200,1.0)

rToy <- extractPlot(resToy,ti="FABIASP",Y=Y)
...
resBreast <- fabiasp(X,5,0.6,200,1.0)

rBreast <- extractPlot(resBreast,ti="FABIASP Breast cancer(Veer)")
...
resMulti <- fabiasp(X,5,0.6,200,1.0)

rMulti <- extractPlot(resMulti,"ti=FABIASP Multiple tissues(Su)")
...
resDLBCL <- fabiasp(X,5,0.6,200,1.0)

rDLBCL <- extractPlot(resDLBCL,ti="FABIASP Lymphoma(Rosenwald)")
...
rToy <- extractPlot(resToy,ti="NMFDIV",Y=Y)
...
rToy <- extractPlot(resToy,ti="NMFEU",Y=Y)
...
rToy <- extractPlot(resToy,ti="NMFSC",Y=Y)
...
resEx <- fabia(X,p,0.1,400,1.0,1.0)
...
resEx <- fabia(X,3,0.1,100,1.0,1.0)
...
resEx <- fabia(X,3,0.1,20,1.0,1.0)
...
resToy <- fabia(X,13,0.1,200,1.0,1.0)
...
resBreast <- fabia(X,5,0.1,200,1.0,1.0)
...
\item Return Values:
 \begin{itemize}
  ...
  \item{pmZ:} permutation matrix of $z$ from kmeans clustering.
  \item{pmL:} permutation matrix of $L$ from kmeans clustering.
  \item{nL:} normalized loadings (left matrix).
  \item{nZ} normalized factors (right matrix).
  \item{Xord:} sorted original matrix according to kmeans on $Z$ and
  kmeans on  $L$.
\end{itemize}
...
resEx <- fabia(X,3,0.3,20,1.0,1.0)
...
\end{enumerate}
  ...
  Program has been obtained by
 \url{http://www.phaget4.org/R/matrixImagePlot.R}
