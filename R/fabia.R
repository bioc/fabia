...
fabia <- function(X,p=5,alpha=0.1,cyc=500,spl=0.5,spz=0.5,random=1.0,center=2,norm=1,scale=0.0,lap=1.0){
	...
	res <- .Call("fabic", X,Psi,L,lapla,cyc ,alpha,eps,eps1,spl,spz,scale,lap,PACKAGE="fabia")
        ...
        for (j in 1:l){
                ppL <- crossprod(nL,ipsi*nL)
                mat <- idp + ppL/res$lapla[j,]
                ini[j,p+1] <- log(det(mat))

                for (i in 1:p){
                    ini[j,i] <- log(1+(1/res$lapla[j,i])*crossprod(nL[,i],ipsi*nL[,i]))
                }
            }
        for (i in 1:p){
            avini[i] <- sum(ini[,i])
        ...
        rownames(nL) <- rowna
        colnames(nL) <- colnames(nL, do.NULL = FALSE, prefix = "bicluster")
        clnames <- colnames(nL)
...
#                L[which(abs(sum1)<alpha)] <- 0
        ...
        for (j in 1:l){
                ...
                for (i in 1:p){
                    ini[j,i] <- log(1+(1/lapla[j,i])*crossprod(nL[,i],ipsi*nL[,i]))
                }
            }
        for (i in 1:p){
            avini[i] <- sum(ini[,i])
...
extractPlot <- function(fact,thresZ=0.5,ti="",thresL=NULL,Y=NULL,which=c(1,2,3,4,5,6,7,8)){
    ...
    if (showf[5]){
        matrixImagePlot(abs(nL),title=paste(ti,": absolute loadings\n",tt,sep=""),yLabels= yll)
    }
    if (showf[6]){
    ...
    return(list(biclust=biclust,pmZ=pmZ,pmL=pmL,L=nL,Z=nZ,Xord=reord))
...
extractBic <- function(fact,thresZ=0.5,thresL=NULL)
{
    ...
    if (is.null(thresL)) {
        mom <- 0
        for (i in 1:p) {
            tmom <- as.numeric(tcrossprod(nL[,i],nZ[i,]))
            mom <- mom + sum(tmom^2)
        }
    ...
    for (i in 1:p){
        ...
        snl <- sort(abs(nL[,i]),decreasing = TRUE,index.return = TRUE)
        gene_namest <- gene_names[snl$ix]
        sL <- nL[snl$ix,i]


        numng[[i]] <- as.vector(which(abs(nL[,i])>thresL))
